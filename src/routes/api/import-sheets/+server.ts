  ...
  try {
    ...
    try {
      ...
      teamStatusMap = new Map(
        legendRows
          .filter((r) => r?.[0] && r?.[1])
          .map(([tag, status]) => [String(tag).trim().toUpperCase(), String(status).trim().toLowerCase()])
      );
    ...
    for (const tab of sheetTabs) {
      ...
      const { data: dataset, error: datasetErr } = await supabaseAdmin
        .from('datasets')
        .upsert({ label: tab, type: 'week', period_start: start, period_end: end }, { onConflict: 'label' })
        .select('id')
        .single();
