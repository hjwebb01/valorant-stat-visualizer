...
if (!CSV_PATH) {
	console.error(
		'Usage: npm run import-valorant -- ./data/file.csv --type=week|season|manual --start=YYYY-MM-DD --end=YYYY-MM-DD [--season=S10] [--label=...]'
	);
	process.exit(1);
}
...
const label =
	arg('label') || (type === 'season' ? (season ?? 'season') : (periodStart ?? 'dataset'));
...
async function main() {
	...
	try {
       ...
       returning id`,
		...
		const parser = createReadStream(CSV_PATH).pipe(parse({ columns: true, bom: true, trim: true }));
      ...
      `;
		...
		console.log(
			`CSV import complete ✅  (dataset: ${label} | ${type} ${periodStart}..${periodEnd})`
		);
	} catch (e) {
